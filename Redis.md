...
解决方案：把恶意攻击查询为空的结果也缓存到redis中，如果发生了增加操作，则替换redis中的数据
...
```bash
vim /root/sentinel.conf		#创建哨兵配置文件

#添加如下代码
sentinel monitor 哨兵组的名字 主节点ip 主节点端口 哨兵数量		#最好奇数个哨兵 最好是三个 投票决定主节点是否宕机
sentinel monitor aa sunshuqiang.top 6379 2		#例子
...
集群内有三个节点（同样建议是奇数节点，利于投票），数据被一分为三，如果集群内一节点宕机，且宕机节点没有从节点，那么集群将进入不可用状态

个个节点之间有心跳机制，采用二进制ping pong机制

宕机的评判标准：集群中有半数以上认为该节点宕机

redis最多为16384个hash槽，hash槽会被均分到集群内的个个节点，所以redis集群最多为16384个节点

当客户端向集群发起请求时，会对key执行CRC16算法，计算结果范围一定在0-16383之内，同一个key在多次计算后结果是一致的，不同的key再计算后结果可能会一致的 ——hash散列

搭建redis集群最少为6台机器，三台主节点带着三台从节点
